...
using namespace ns3;
NS_LOG_COMPONENT_DEFINE ("LispNatProxyHandoverTestSuite");

class LispNatProxyHandoverTestCase : public TestCase
{
public:
  LispNatProxyHandoverTestCase ();
  virtual ~LispNatProxyHandoverTestCase ();
...
LispNatProxyHandoverTestCase::LispNatProxyHandoverTestCase ()
  : TestCase ("MN LISP test case: checks lisp -> nat handover"), m_receivedPacket_first_interface(false), m_receivedPacket_second_interface(false)
{
}
...
void 
LispNatProxyHandoverTestCase::RxSink (Ptr<const Packet> p)
{
...
void
LispNatProxyHandoverTestCase::DoRun (void)
{
  ...
  */
  ...
  //LogComponentEnable ("LispNatProxyHandoverTestSuite", LOG_LEVEL_INFO);
  ...
  Time MNAppStart = Seconds(4.0); /* Need to start later, to allow registration 
  ...
  NodeContainer R1_xTR1 = NodeContainer (nodes.Get (2), nodes.Get (3));
  NodeContainer R1_NAT = NodeContainer (nodes.Get (2), nodes.Get (4));
  NodeContainer NAT_R3 = NodeContainer (nodes.Get (4), nodes.Get (5));
  NodeContainer xTR1_MN = NodeContainer (nodes.Get (3), nodes.Get (11));
  ...
  NodeContainer lispRouters = NodeContainer (nodes.Get (6), nodes.Get (7), nodes.Get (MN), nodes.Get (3));
  lispRouters.Add (nodes.Get (8)); // RTR
  lispRouters.Add (nodes.Get (9)); // PETR
  lispRouters.Add (nodes.Get (10)); // PITR
  NodeContainer xTRs = NodeContainer (nodes.Get (8), nodes.Get (9), nodes.Get (10), nodes.Get (3));
  NodeContainer MNs = NodeContainer(nodes.Get (MN));
  ...
  lispHelper.BuildRlocsSet ("src/internet/test/lisp-test/lisp-nat-proxy-handover/lisp_nat_proxy_handover_lisp_rlocs.txt");
  lispHelper.SetRtrs(NodeContainer(nodes.Get (8)));
  lispHelper.Install(lispRouters);
  NS_LOG_INFO("Lisp router installed");
  lispHelper.BuildMapTables2("src/internet/test/lisp-test/lisp-nat-proxy-handover/lisp_nat_proxy_handover_lisp_rlocs_config_xml.txt");
  ...
  lispMNAppHelper.AddMapServerAddress (static_cast<Address> (iR1_iMS.GetAddress (1)));
  ApplicationContainer MNApps = lispMNAppHelper.Install (MNs);
  MNApps.Start (MNAppStart);
  MNApps.Stop (EndTime);
  ...
  clientApps.Get (0)->TraceConnectWithoutContext ("Rx", MakeCallback (&LispNatProxyHandoverTestCase::RxSink, this));
  
  /* -------------------------------------- *\
                TRACING
  \* -------------------------------------- */
  //p2p.EnablePcapAll ("lisp_nat_proxy_handover_test_suite");
...
// ===================================================================================
class LispNatProxyHandoverTestSuite : public TestSuite
{
public:
 LispNatProxyHandoverTestSuite ();
};
LispNatProxyHandoverTestSuite::LispNatProxyHandoverTestSuite ()
  : TestSuite ("lisp-nat-proxy-handover", UNIT)
{
  AddTestCase (new LispNatProxyHandoverTestCase, TestCase::QUICK);
}

static LispNatProxyHandoverTestSuite lispNatProxyHandoverTestSuite;