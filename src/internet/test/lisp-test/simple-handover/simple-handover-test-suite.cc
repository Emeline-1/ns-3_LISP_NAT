...
using namespace ns3;
NS_LOG_COMPONENT_DEFINE ("SimpleHandoverTestSuite");

class SimpleHandoverTestCase : public TestCase
{
public:
  SimpleHandoverTestCase ();
  virtual ~SimpleHandoverTestCase ();
...
private:
  ...
  Time m_handTime = Seconds(10.5);
  void RxSink (Ptr<const Packet> p);
};

SimpleHandoverTestCase::SimpleHandoverTestCase ()
  : TestCase ("MN LISP test case: checks simple handover"), m_receivedPacket_first_interface(false), m_receivedPacket_second_interface(false)
{
}
...
void 
SimpleHandoverTestCase::RxSink (Ptr<const Packet> p)
{
...
void
SimpleHandoverTestCase::DoRun (void)
{
  ...
  */
  ...
  LogComponentEnable ("SimpleHandoverTestSuite", LOG_LEVEL_INFO);
  ...
  lispHelper.BuildRlocsSet ("src/internet/test/lisp-test/simple-handover/simple_handover_lisp_rlocs.txt");
  lispHelper.Install(lispRouters);
  NS_LOG_INFO("Lisp router installed");
  lispHelper.BuildMapTables2("src/internet/test/lisp-test/simple-handover/simple_handover_lisp_rlocs_config_xml.txt");
  ...
  clientApps.Get (0)->TraceConnectWithoutContext ("Rx", MakeCallback (&SimpleHandoverTestCase::RxSink, this));
...
// ===================================================================================
class SimpleHandoverTestSuite : public TestSuite
{
public:
 SimpleHandoverTestSuite ();
};
SimpleHandoverTestSuite::SimpleHandoverTestSuite ()
  : TestSuite ("simple-handover-lisp", UNIT)
{
  AddTestCase (new SimpleHandoverTestCase, TestCase::QUICK);
}

static SimpleHandoverTestSuite simpleHandoverTestSuite;