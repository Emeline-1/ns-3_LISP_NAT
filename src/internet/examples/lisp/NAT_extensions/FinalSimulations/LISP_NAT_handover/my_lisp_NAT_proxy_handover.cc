...
NS_LOG_COMPONENT_DEFINE ("LISPNATProxyHandover");
...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  if (verbose)
  {
    ...
    //LogComponentEnable ("LISPNATProxyHandover", LOG_LEVEL_INFO);
  ...
  lispHelper.BuildRlocsSet ("./lisp_nat_proxy_handover_lisp_rlocs.txt");
  lispHelper.SetRtrs(NodeContainer(nodes.Get (8)));
  lispHelper.Install(lispRouters);
  NS_LOG_INFO("Lisp router installed");
  lispHelper.BuildMapTables2("./lisp_nat_proxy_handover_lisp_rlocs_config_xml.txt");
  ...
  */
  ...
  /* -------------------------------------- *\
                TRACING
  \* -------------------------------------- */
  p2p.EnablePcapAll ("lisp_nat_proxy_handover");
