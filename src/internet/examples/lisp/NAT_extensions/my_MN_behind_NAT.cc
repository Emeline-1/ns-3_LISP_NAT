...
NS_LOG_COMPONENT_DEFINE ("MNBehindNAT");
...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  LogComponentEnable ("MNBehindNAT", LOG_LEVEL_INFO);
  ...
  lispHelper.BuildRlocsSet ("./MN_behind_NAT_lisp_rlocs.txt");
  lispHelper.SetRtrs(NodeContainer(nodes.Get (8)));
  lispHelper.Install(lispRouters);
  NS_LOG_INFO("Lisp router installed");
  lispHelper.BuildMapTables2("./MN_behind_NAT_lisp_rlocs_config_xml.txt");
  ...
  /* -------------------------------------- *\
                SIMULATION
  \* -------------------------------------- */
  p2p.EnablePcapAll ("MN_behind_NAT");
