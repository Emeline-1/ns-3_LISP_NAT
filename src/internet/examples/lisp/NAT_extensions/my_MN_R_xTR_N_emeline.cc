...
NS_LOG_COMPONENT_DEFINE ("MN_R_xTR_N");
...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  LogComponentEnable ("MN_R_xTR_N", LOG_LEVEL_INFO);
  ...
  LogComponentEnable ("NetDevice", LOG_LEVEL_DEBUG);
  ...
  lispHelper.BuildRlocsSet ("./MN_R_xTR_N_lisp_rlocs.txt");
  lispHelper.Install(lispRouters);
  //lispHelper.BuildMapTables2("./scratch/simple_lisp_rlocs_config_xml.txt");
  lispHelper.BuildMapTables2("./MN_R_xTR_N_lisp_rlocs_config_xml.txt");
  lispHelper.InstallMapTables(lispRouters);

  NS_LOG_INFO("Installing Control Plane...");
  // Control Plane
